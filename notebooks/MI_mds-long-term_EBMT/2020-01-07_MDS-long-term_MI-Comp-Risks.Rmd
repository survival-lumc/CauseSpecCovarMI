...
LdW_schetelig_subset <- read.spss("liesbeth_files/mds_longterm_aaauto.sav",
                                  to.data.frame = T)
...
dat_mds <- dat_mds %>% 
  ...
  mutate(time = ci_allo1,
         ci_s_allo1 = case_when(
           is.na(ci_s_allo1) & srv_s_allo1 == "dead" ~ "nrm",
           is.na(ci_s_allo1) & srv_s_allo1 == "alive" ~ "rfs",
           !is.na(ci_s_allo1) ~ as.character(ci_s_allo1)
         ),
         ci_s_allo1 = factor(ci_s_allo1, levels = c("rfs", "relapse", "nrm")),
         delta = as.numeric(ci_s_allo1) - 1,
...
plot(cumin_rel_nrm, xlab = "Months", col = c("blue", "black"), 
     curvlab = c("Relapse", "NRM"), xlim = c(0, 120))
