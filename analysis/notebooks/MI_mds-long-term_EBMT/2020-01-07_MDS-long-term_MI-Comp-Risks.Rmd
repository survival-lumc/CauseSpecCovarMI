...
centre_dat <- read.spss(
  "analysis/data/raw_data/MDS_longterm_R_180614.sav",
  to.data.frame = T,
  use.value.labels = T,
  use.missings = T
) %>% 
...
p <- gg_miss_var(dat_mds, show_pct = T) +
  theme_bw(base_size = 30) + xlab(NULL)
miss_var_summary(dat_mds)

library("svglite")
ggsave("mds_long-term_missings.svg", dpi = "retina", units = "in",
       width = 8, height = 10)
...
dat_mds_aft <- dat_mds %>% 
  ...
  mutate(time = ci_allo1 / 12) %>% # Put in yrs
  ...
  mutate(time = ifelse(time > 10, 10, time),
         delta = ifelse(time == 10, 0,
...
bind_rows(ev1, ev2) %>% 
  ggplot(aes(time, hazard, col = factor(ev))) +
  geom_point()
...
# Try smcfcs with year last fu as multilevel factor var
...
dat_mds_MI <- dat_mds_aft %>% 
  select_keeplabs("age_allo1", "AGEDONOR_allo1_1", "time", 
                  "delta", "stat1", "stat2", "CENTRE_allo1",
                  "mdsclass", "donorrel", "ric_allo1",
                  "bm_allo1") %>% 
  # Compute baselines hazards
  mutate(H1 = nelsonaalen(., time, stat1),
         H2 = nelsonaalen(., time, stat2),
         delta = factor(delta),
         CENTRE_allo1 = factor(CENTRE_allo1, ordered = F),
         mdsclass = droplevels(mdsclass))

library(data.table)

data.table(dat_mds_MI) %>% 
  .[, .("N" = .N) , by = CENTRE_allo1] %>% 
  View()
...
# Set methods accordingly, mlogit
meth_miss <- setNames(c("norm", "logreg", "norm"), var_names_miss)
...
# We don't use time is imputation models, justuse delta as factor
matpred[, c("time", "stat1", "stat2")] <- 0
matpred[, c("H1", "H2", "delta")] <- 0

matpred

# We only imputer donor age
matpred[!(rownames(matpred) %in% c("AGEDONOR_allo1_1", 
                                   "ric_allo1",
                                   "bm_allo1")), ] <- 0

test <- smcfcs(originaldata = dat_mds_MI, 
               smtype = "compet", 
               smformula = c("Surv(time, delta == 1) ~ age_allo1 + AGEDONOR_allo1_1 + bm_allo1 + ric_allo1 + mdsclass + donorrel",
                             "Surv(time, delta == 2) ~ age_allo1 + AGEDONOR_allo1_1 + bm_allo1 + ric_allo1 + mdsclass + donorrel"),
               method = meths, 
               predictorMatrix = matpred, 
               m = 5)

plot(test$smCoefIter[, 1, 10])

# Things that go wrong:
# - don't include centre in substantive model - the cause-specific cox wont converge
# - Pick correct imputation methods
# - If you have subsetted factors, make sure all levels are non-zero
# just use droplevels() to be safe
# - No problem however including centre in the imputation models, specify it
# in matpred before hand
```
