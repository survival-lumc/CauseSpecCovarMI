...
# Synth
mod_rel
mod_rel_syn <- coxph(formula = update(form_rel, new = Surv(t_ev1, ev1) ~ .), 
      data = dato_s$syn)

dev.new()
ggforest(mod_rel, data = dato)

dev.new()
ggforest(mod_rel_syn, data = dato)
#dato_s <- syn(dato, seed = 1023)

dato <- copy(dat_mds_reg)
dato[, ':=' (
  aa_auto = NULL,
  t_ev1 = ifelse(ci_s_allo1 == 1, ci_allo1, srv_allo1),
  t_ev2 = ifelse(ci_s_allo1 == 2, ci_allo1, srv_allo1),
  ev1 = as.numeric(ci_s_allo1 == 1),
  ev2 = as.numeric(ci_s_allo1 == 2),
  srv_s_allo1 = NULL,
  srv_allo1 = NULL,
  ci_s_allo1 = NULL,
  ci_allo1 = NULL
...
colnames(dato)
meths <- c("sample", rep("cart", ncol(dato) - 1))
names(meths) <- colnames(dato)
meths[c(12, 13)] <- "survctree" # 13
meths[c("ev1", "ev2")] <- ""
meths
...
dato_s <- synthpop::syn(
  data = dato,
  method = meths, 
  visit.sequence = c(1, 3, 4, 11, 2, 6, 7, 
                     5, 10, 9, 8, 12, 13), # order of less > more missing
  proper = TRUE, 
  models = TRUE,
...
mod_nrm
mod_nrm_syn <- coxph(formula = update(form_rel, new = Surv(t_ev2, ev2) ~ .), 
      data = dato_s$syn)


dev.new()
ggforest(mod_nrm, data = dato)

dev.new()
ggforest(mod_nrm_syn, data = dato)
