...
Given standard normal $Z$ and a standard normal or binary $X$, event times were generated using the following latent parametrisation:
...
where we varied $\beta_1 = \{0, 0.5, 1\}$, and fixed $\gamma_1 = 1$, $\beta_2 = 0.5$ and $\gamma_2 = 0.5$. 

Concerning the baseline hazard shapes, there were two parametrisations: 'similar' and 'different'. For the former, the values were defined as 
...
In the 'different' case, we set $\{\alpha_1,\lambda_{10}\} = \{1.5,0.04\}$.

What do these baseline hazards look like, and what are the baseline cumulative incidences?
...
Recall that there were four difference missingness mechanisms for $X$:
...
Setting $\eta_1 = -2$, we can visualise each of them as follows:
