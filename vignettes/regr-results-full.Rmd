...
mcse_covarage <- regr_results[m %in% c(0, 50) & n == 2000 & beta1 != "0" & 
                            X_level == "continuous"][["cover_mcse"]]
```

- Range of Bias MCSE: [`r round(min(mcse_bias), 3)`; `r round(max(mcse_bias), 3)`]
- Range of RMSE MCSE: (to come)
- Range of Coverage MCSE: [`r round(min(mcse_covarage), 3)`; `r round(max(mcse_covarage), 3)`]
- Range of estimate standard error MCSE: (to come)
...
mcse_covarage_bin <- regr_results[m %in% c(0, 50) & n == 2000 & beta1 != "0" & 
                            X_level == "binary"][["cover_mcse"]]
```

- Range of Bias MCSE: [`r round(min(mcse_bias_bin), 3)`; `r round(max(mcse_bias_bin), 3)`]
- Range of RMSE MCSE: (to come)
- Range of Coverage MCSE: [`r round(min(mcse_covarage_bin), 3)`; `r round(max(mcse_covarage_bin), 3)`]
- Range of estimate standard error MCSE: (to come)
