...
file_ref<-"/home/mresche/mi_cmprsk/resultat"
file_result<-"essai01_we_sans_cens_bin_mar_26_01_2015"

setwd(file_ref)
dir.create(paste(file_result,sep=""))
file.copy("/home/mresche/mi_cmprsk/script/simulcs_we_avec_cens.r", paste(file_result,"/simulcs_we_avec_cens.r",sep=""))
file.copy("/home/mresche/mi_cmprsk/script/tiragecs_weib.r", paste(file_result,"/tiragecs_weib.r",sep=""))

tmp.file<-list.files()

if (file_result%in%tmp.file==FALSE)
{
  dir.create(file_result)
}

source("/home/mresche/mi_cmprsk/script/tiragecs_weib.r")
...
for (i in 1:repet)
{
  ...
  # Tirage
  baz<-tiragecs_we(n=280,h10=1,h20=0.5,a1=0.3,a2=1.7,b1X=beta1X,b2X=beta2X,
                   b1Z=beta1Z,b2Z=beta2Z,
                   fXZ=function(nn){
                     #tmp<-mvrnorm(n=nn,mu=c(0,0),Sigma=matrix(c(1,0,0,1),nrow=2))
                     b0<-0
                     b1<--2
                     ZZ<-rbinom(nn,1,0.5)-0.5
                     data.frame(X=rbinom(nn,1,exp(b0+b1*ZZ)/(1+exp(b0+b1*ZZ)))-0.5,Z=ZZ)
                   },
                miss=function(nn,zz){
                  tmp.nn<-table(zz>0)
                  ifelse(zz>0,rbinom(tmp.nn[2],1,0.1),rbinom(tmp.nn[1],1,0.9))},
                censure=list(tirc=function(nn,hc){rexp(nn,hc)},varc=0.0000001))
  baz$Xm<-as.factor(baz$Xm)
...
#                       log(res.ref.2$conf.int["Z",c("lower .95","upper .95")])))
  ...
  cc.1<-rbind(cc.1,c(res.cc.1$coefficients["Xm1",c("coef","se(coef)","Pr(>|z|)")],
                     log(res.cc.1$conf.int["Xm1",c("lower .95","upper .95")]),
...
#   
  ...
  tmp.imp$method["Xm"]<-"logreg"
  tmp.imp$pred[,c("X","t1","t2","tc","tt","missind","ee1","ee2","H02Z","H01Z")]<-0
  tmp.imp<-mice(baz,m=10,maxit=maxx,predictorMatrix=tmp.imp$pred,
                method=tmp.imp$method,print=FALSE)
  tmp.ana.imp<-with(tmp.imp,expr=coxph(Surv(tt,ee1)~Xm+Z))
  res.impc.1<-summary(pool(tmp.ana.imp))
  ...
  tmp.imp<-mice(baz,m=10,maxit=maxx,predictorMatrix=tmp.imp$pred,
                method=tmp.imp$method,print=FALSE)
  tmp.ana.imp<-with(tmp.imp,expr=coxph(Surv(tt,ee1)~Xm+Z))
  res.impc.2<-summary(pool(tmp.ana.imp))

  impc.1<-rbind(impc.1,c(res.impc.1["Xm1",c("est","se","Pr(>|t|)","lo 95","hi 95")],
                         res.impc.1["Z",c("est","se","Pr(>|t|)","lo 95","hi 95")]))
  impc.2<-rbind(impc.2,c(res.impc.2["Xm1",c("est","se","Pr(>|t|)","lo 95","hi 95")],
                         res.impc.2["Z",c("est","se","Pr(>|t|)","lo 95","hi 95")]))
  ...
  tmp.imp<-mice(baz,m=10,maxit=1,predictorMatrix=tmp.imp$pred,
                method=tmp.imp$method,print=FALSE)
  tmp.ana.imp<-with(tmp.imp,expr=coxph(Surv(tt,ee1)~Xm+Z))
  res.imp1.1<-summary(pool(tmp.ana.imp))
...
#   res.imp2.2<-summary(pool(tmp.ana.imp))
  imp1.1<-rbind(imp1.1,c(res.imp1.1["Xm1",c("est","se","Pr(>|t|)","lo 95","hi 95")],
                         res.imp1.1["Z",c("est","se","Pr(>|t|)","lo 95","hi 95")]))
